...
// Not <cpuid.h> - this project's own src/xrCore/cpuid.h (an unrelated,
// legacy MSVC-CRT-style header of the same name) shadows the real compiler
// one on the include search path, so the __cpuid/__cpuid_count macros are
// reimplemented directly here instead.
#if defined(__i386__) && defined(__PIC__)
#define _XM_CPUID(level, a, b, c, d)                                                                                 \
	__asm__ __volatile__("xchg{l}\t{%%}ebx, %1\n\t"                                                                  \
	                     "cpuid\n\t"                                                                                 \
	                     "xchg{l}\t{%%}ebx, %1\n\t"                                                                  \
	                     : "=a"(a), "=r"(b), "=c"(c), "=d"(d)                                                        \
	                     : "0"(level))
#define _XM_CPUID_COUNT(level, count, a, b, c, d)                                                                    \
	__asm__ __volatile__("xchg{l}\t{%%}ebx, %1\n\t"                                                                  \
	                     "cpuid\n\t"                                                                                 \
	                     "xchg{l}\t{%%}ebx, %1\n\t"                                                                  \
	                     : "=a"(a), "=r"(b), "=c"(c), "=d"(d)                                                        \
...
#endif
#define __cpuid(level, a, b, c, d) _XM_CPUID(level, a, b, c, d)
#define __cpuid_count(level, count, a, b, c, d) _XM_CPUID_COUNT(level, count, a, b, c, d)
#endif
