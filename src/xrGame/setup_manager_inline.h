...
{
	typename CSSetupManager::setup_actions::const_iterator I = std::find_if(actions().begin(), actions().end(), setup_pred(action_id));
...
IC void CSSetupManager::select_action()
{
	if (!m_actuality || current_action().completed())
	{
		...
		typename CSSetupManager::setup_actions::const_iterator I = actions().begin();
		typename CSSetupManager::setup_actions::const_iterator E = actions().end();
		for (; I != E; ++I)
