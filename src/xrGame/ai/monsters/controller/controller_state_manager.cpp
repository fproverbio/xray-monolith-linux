...
{
	this->add_state(eStateRest, xr_new<CStateMonsterRest<CController>>(obj));
	this->add_state(eStatePanic, xr_new<CStateMonsterPanic<CController>>(obj));
	this->add_state(eStateHearInterestingSound, xr_new<CStateMonsterHearInterestingSound<CController>>(obj));
	this->add_state(eStateHearDangerousSound, xr_new<CStateMonsterHearDangerousSound<CController>>(obj));
	this->add_state(eStateHitted, xr_new<CStateMonsterHitted<CController>>(obj));
	this->add_state(eStateAttack, xr_new<CStateControllerAttack<CController>>(obj));

	this->add_state(eStateEat, xr_new<CStateMonsterEat<CController>>(obj));
	this->add_state(eStateCustom, xr_new<CStateControlHide<CController>>(obj));
}
...
bool CStateManagerController::check_control_start_conditions(ControlCom::EControlType type)
{
	...
	{
		return this->current_substate == eStateAttack_Run;
	}
