...
bool CLevel::net_start1()
{
	...
	if (m_caServerOptions.size())
	{
		...
		// Connect
		// Singleplayer-only fork: p.m_game_type is always "single" (GameID() is
		// always eGameIDSingle here), and the MP xrGameSpyServer branch that used
		// to run otherwise is dead code - the whole xrGameSpyServer class/header
		// was removed from this fork along with GameSpy/DirectPlay8 multiplayer,
		// see commit 1d4ec53e.
