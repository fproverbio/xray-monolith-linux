...
TEMPLATE_SPECIALIZATION
IC void CProblemSolverAbstract::add_operator(const _edge_type& operator_id, _TOperatorPtr _operator_arg)
{
...
#ifdef DEBUG
	validate_properties			(_operator_arg->conditions());
	validate_properties			(_operator_arg->effects());
#endif
	m_actuality = false;
	m_operators.insert(I, SOperator(operator_id, _operator_arg));
}
...
{
	return (estimate_edge_weight_impl_dispatch<reverse_search>(condition));
}
