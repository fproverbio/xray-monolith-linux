...
void CWeapon::FireTrace(const Fvector& P, const Fvector& D)
{
	...
	float fire_disp = 0.f;
	// First-bullet-accuracy correction and the game_cl_mp reward-generator hook
	// were both MP-only (gated behind !IsGameTypeSingle(), always false here) -
	// dead in this singleplayer-only port, see commit 1d4ec53e.
