...
#include "UIGameLog.h"
// UIChatWnd.h removed - see UIMessagesWindow.h: genuinely absent anywhere in
// this source tree, part of the removed GameSpy/MP subsystem.
...
void CUIMessagesWindow::Init(float x, float y, float width, float height)
{
	...
	// Originally `if (IsGameTypeSingle()) {...} else { <MP chat window setup> }`
	// - the MP branch (CUIChatWnd/m_pChatWnd, chat_log_list) is unconditionally
	// dropped: UIChatWnd.h genuinely does not exist anywhere in this source
	// tree (removed along with the rest of the GameSpy/MP subsystem, see
	// UIMessagesWindow.h). This narrows real behavior (chat log/pending-mode
	// members are now always inert) but SP behavior is unchanged.
...
{
	// m_pChatWnd/CUIChatWnd removed - see header.
	if (m_pGameLog)
