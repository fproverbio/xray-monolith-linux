...
IC void CSolverPathManager::create_path(T& vertex)
{
	VERIFY(this->data_storage);
	if (m_edge_path)
		// _Graph::reverse_search is an enum VALUE (see CProblemSolver's
		// anonymous enum), not a type - `typename` here was illegal.
		this->data_storage->get_edge_path(*m_edge_path, &vertex, bool(_Graph::reverse_search));
}
