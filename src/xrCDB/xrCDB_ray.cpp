...
// ray_collider actually unaligned. isect_sse() below loads them with
// _mm_load_ps (aligned movaps), which faults ("illegal storage access")
// whenever the address isn't 16-byte aligned. alignas(16) fixes the
// struct/class uses below but GCC silently *ignores* it (with
// -Wattributes) when it lands between a type-specifier and a declarator,
// which is exactly how this macro is also used further down for
// "static const float _MM_ALIGN16 ps_cst_plus_inf[4]" - so use the GNU
// attribute form instead, which applies correctly in both positions.
#ifndef _MM_ALIGN16
